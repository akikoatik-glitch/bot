...
async function cmdHelp({ chatId }) {
  const msg = [
    ...
    '',
    '<b>النشر اليومي</b>',
    '/morning — إرسال التحية الصباحية فوراً',
    '/plan — التخطيط لتوقعات اليوم فوراً',
    '/summary — إرسال الملخص المسائي فوراً',
    '',
    '<b>القواعد</b>',
...
async function cmdMorning({ chatId }) {
  await tg.sendMessage(chatId, '⏳ جاري إرسال التحية الصباحية…', { throttle: false });
  const r = await daily.greetingIfNeeded({ force: true });
  await tg.sendMessage(chatId, `✅ حدث التحية: <b>${r.sent ? 'أُرسلت' : 'لم تُرسل'}</b>\nالسبب: <b>${escapeHtml(r.reason || 'ok')}</b>`, { throttle: false });
}

async function cmdPlan({ chatId }) {
  await tg.sendMessage(chatId, '⏳ جاري التخطيط لتوقعات اليوم…', { throttle: false });
  const r = await daily.planDay({ force: true });
  await tg.sendMessage(chatId, `✅ الخطة: <b>${r.planned}</b> توقع مُخطط له\nالسبب: <b>${escapeHtml(r.reason || 'ok')}</b>`, { throttle: false });
}

async function cmdSummary({ chatId }) {
  await tg.sendMessage(chatId, '⏳ جاري إرسال الملخص…', { throttle: false });
  const r = await daily.dailySummary({ force: true });
  await tg.sendMessage(chatId, `✅ الملخص: <b>${r.sent ? 'أُرسل' : 'لم يُرسل'}</b>\nالسبب: <b>${escapeHtml(r.reason || 'ok')}</b>`, { throttle: false });
}
