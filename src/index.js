...
async function main() {
  ...
  // First daily tick after a brief warm-up (plan → greeting → publish → summary)
  setTimeout(async () => {
    try {
      log.info('bot.first_tick.start');
      const r = await daily.tick();
      log.info('bot.first_tick.done', r);
      try { await seo.regenerate(); } catch (e) { log.warn('seo.init.failed', { err: e.message }); }
    } catch (e) {
      log.error('bot.first_tick.failed', { err: e.message });
    }
  }, 5000);
