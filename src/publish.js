...
async function publishCycle({ dryRun = false } = {}) {
  ...
  // Recent best-bet markets (variety guard: avoid posting the same market 3x in a row)
  ...
  for (const m of upcoming) {
    try {
      ...
      if (result.best_bet && result.best_bet.key) recentKeys.push(result.best_bet.key);
      enriched.push({
  ...
  // Filter & rank on the BEST market (not 1X2 alone — this was the "always
  // win or draw" bias: matches with a strong goals/BTTS signal but uncertain
  // 1X2 never made it through).
  const eligible = enriched.filter(e => (e.result.best_bet ? e.result.best_bet.prob : e.result.conf) >= config.scheduler.minConfidence);
  ...
  for (const e of top) {
    ...
    try {
      ...
      db.savePrediction({
        ...
        best_bet_key: r.best_bet ? r.best_bet.key : null,
        best_bet_label: r.best_bet ? r.best_bet.label : null,
        best_bet_prob: r.best_bet ? r.best_bet.prob / 100 : null,
        arabic_text: text,
        english_text: null,
      ...
      log.info('publish.ok', {
        ...
        bestBet: r.best_bet ? `${r.best_bet.key}:${r.best_bet.prob}` : null,
        home: m.home_name, away: m.away_name,
        kickoff: m.utc_date,
      });
